...
import { LinkwardenConfig } from "./types.js";

function getConfig(): LinkwardenConfig {
  const baseUrl = process.env.LINKWARDEN_URL;
  const apiToken = process.env.LINKWARDEN_TOKEN;

  if (!baseUrl) throw new Error("LINKWARDEN_URL environment variable is required");
  if (!apiToken) throw new Error("LINKWARDEN_TOKEN environment variable is required");
...
const server = new Server(
  { name: "mcp-linkwarden", version: "0.1.0" },
  { capabilities: { resources: {}, tools: {} } },
);

function highlightToMarkdown(h: any, linkName?: string): string {
  ...
  switch (uri) {
    case "linkwarden://highlights": {
      ...
      const md = highlights.length === 0
        ? "# Highlights\n\nNo highlights found."
        : "# Highlights\n\n" + highlights.map((h) =>
            highlightToMarkdown(h, linkMap.get(h.linkId))
          ).join("\n---\n");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case "get_link_highlights": {
      const { linkId } = args as { linkId: number };
      const highlights = await client.getHighlightsByLink(linkId);
      const link = await client.getLink(linkId);
    ...
    case "export_highlights": {
      const { collectionId } = (args || {}) as { collectionId?: number };
