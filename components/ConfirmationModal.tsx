					...
					<Drawer.Content className="flex flex-col rounded-t-2xl h-[90%] mt-24 fixed bottom-0 left-0 right-0 z-30 outline-none">
						...
						>
							...
							<div className="w-full flex items-center justify-end gap-2 mt-3">
								<Button intent="ghost" className="hover:bg-base-200" onClick={() => toggleModal()}>
									{t("cancel")}
								</Button>
								<Button intent="destructive" onClick={async () => {
									await onConfirmed();
									toggleModal();
								}}>
									{t("confirm")}
		...
		return (
			...
			>
				<ClickAwayHandler
					...
					className={clsx("w-full mt-auto sm:m-auto sm:w-11/12 sm:max-w-2xl", className)}
				>
					<div
						className={"slide-up mt-auto sm:m-auto relative border-neutral-content rounded-t-2xl sm:rounded-2xl border-t sm:border shadow-2xl bg-base-100 overflow-y-auto sm:overflow-y-visible p-5"}
