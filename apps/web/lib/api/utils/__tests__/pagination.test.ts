...
describe('parsePagination', () => {
  ...
  it('should throw error for invalid cursor', () => {
    expect(() => parsePagination({ cursor: -5 })).toThrow('Invalid cursor value');
  });

  it('should throw error for NaN cursor', () => {
    expect(() => parsePagination({ cursor: NaN })).toThrow('Invalid cursor value');
  });
...
describe('buildPaginatedResponse', () => {
  ...
  it('should handle exactly limit items', () => {
    const items = Array.from({ length: 50 }, (_, i) => ({ id: i + 1, name: `item${i}` }));
...
describe('buildPaginatedResponseWithCount', () => {
  ...
  it('should work with hasMore true', () => {
    const items = Array.from({ length: 10 }, (_, i) => ({ id: i + 1, name: `item${i}` }));
...
describe('paginate', () => {
  ...
  it('should handle query function errors', async () => {
    ...
    await expect(
      paginate({ limit: 10 }, queryFn)
    ).rejects.toThrow('Database error');
  });

  it('should correctly detect hasMore with full page', async () => {
    const mockData = Array.from({ length: 10 }, (_, i) => ({ id: i + 1, name: `item${i}` }));
    const queryFn = jest.fn().mockResolvedValue(mockData);
