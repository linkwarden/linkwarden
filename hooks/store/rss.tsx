...
const useAddRssSubscription = () => {
	...
	return useMutation({
		...
		onSuccess: (data) => {
			queryClient.setQueryData(["rss-subscriptions"], (oldData: RssSubscription[]) => [
				...oldData,
				data.rssSubscription,
			]);
		},
	})
}
...
const useDeleteRssSubscription = () => {
	...
	return useMutation({
		...
		onSuccess: (_, rssSubscriptionId) => {
			queryClient.setQueryData(["rss-subscriptions"], (oldData: RssSubscription[]) => {
				return oldData.filter((rssSubscription) => rssSubscription.id !== rssSubscriptionId);
			});
		},
	});
}
